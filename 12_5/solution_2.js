...
async function processLineByLine() {
  ...
  for await (const line of rl) {
    ...
    if (line.startsWith("move")) {
      ...
      const cratesToMove = Array(parseInt(numCrates))
        .fill("")
        .reduce((prev) => {
      ...
      const existingStackTo = crateStacks[parseInt(stackTo) - 1];
      crateStacks[parseInt(stackTo) - 1] = existingStackTo.concat(cratesToMove);
      print(crateStacks);
    }
  }
